...
class productController {
    add_product = async (req, res) => {
        ...
        form.parse(req, async (err, field, files) => {
            let { name, category, description, stock, price, discount, shopName, brand, location } = field;
            const { images } = files;
            name = name.trim();
    ...
    product_update = async (req, res) => {
        let { name, description, discount, price, brand, productId, stock, location } = req.body;
        name = name.trim();
        const slug = name.split(' ').join('-');
        try {
            await productModel.findByIdAndUpdate(productId, {
                name, description, discount, price, brand, stock, slug, location
            });
            const product = await productModel.findById(productId);
